      ...
      <table class="table table-bordered" id="current">
        ...
        <?php $i = 0; ?>
          ...
          <tr>        
            <td>{{ $i }} - {{ $data->id }}</td>
            <td>{{ $data->match_id }}
              {{ !empty($detailMatch[$data->match_id]) ? $detailMatch[$data->match_id]['team_name'] : '...' }} - {{ !empty($detailMatch[$data->match_id]) ? $detailMatch[$data->match_id]['team_name2'] : "..." }}
              @if(App\Models\MatchBetDetail::where([
      'half' => $data->time_half,
      'user_id' => $account_id,
      'ref_id' => $data->match_id,

      ])->where('minute', '>=', $data->time_from)->where('minute', '<=', $data->time_to)->count() > 0 )
              <a href="{{ route('view-log', ['schedule_id' => $data->id]) }}" target="_blank">View log</a>
              @endif
            </td>
            <td>
              <?php 
